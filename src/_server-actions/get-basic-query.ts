...
//     ...(filters.headline ? { headline: { $containsi: filters.headline } } : {}),
...
// No seu arquivo get-basic-query.ts, atualize a função:
export async function getBasicQuery(
  filters: BasicFilters,
  queryOverride?: Record<string, any>,
) {
  // Tratar "todos" como undefined/null
  const filterYear =
    filters.ano && filters.ano !== 'todos'
      ? filters.ano
      : filters.mes && filters.mes !== 'todos'
