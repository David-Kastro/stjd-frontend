...
export async function NoticiasTemplate({
  newsData,
  filters,
}: {
  ...
  // Lógica ajustada para quando há filtros aplicados
  let highlightedNews: News[] = []
  let news: News[] = []

  if (hasFilters) {
    // Quando há filtros, mostra todos os resultados no carousel principal
    // e deixa o highlight vazio ou usa apenas o primeiro item
    if (newsData.length > 0) {
      highlightedNews = [newsData[0]] // Apenas o primeiro como destaque
      news = newsData // Todos os resultados no carousel
    }
  } else {
    // Comportamento original quando não há filtros
    highlightedNews = newsData.slice(0, 3)
    news = newsData.slice(3)
  }
