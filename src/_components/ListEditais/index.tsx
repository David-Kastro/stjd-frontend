  ...
  return (
    <div className="mt-[3.81rem] w-full">
      <h1 className="inline-flex items-center gap-[0.56rem] text-[1.25rem] font-bold">
        <FileText />
        {editais.length > 0 && 'numero_acordao' in editais[0]
          ? 'Jurisprudência'
          : 'Editais'}
      </h1>
          ...
          {editais.map((item) => {
            ...
            return (
              <CardEdital
                key={
                  isJurisprudencia
                    ? `jurisprudencia_${item.id}`
                    : `edital_${(item as Edital).documentId}_${item.id}`
                }
                title={
                  isJurisprudencia
                    ? (item as Jurisprudencia).numero_acordao ||
                      (item as Jurisprudencia).titulo
                    : (item as Edital).titulo
                }
                ...
                path={
                  isJurisprudencia
                    ? `/jurisprudencia/jurisprudencia-stjd?documentId=${item.documentId}`
                    : `/processos/editais?documentId=${(item as Edital).documentId}`
                }
              />
            )
          })}
      ...
      <div className="border-[#BFBFBF] pb-[2.88rem] pt-[1.5rem] lg:mr-1 lg:border-r-[2px]">
        <Link
          href={
            editais.length > 0 && 'numero_acordao' in editais[0]
              ? `/jurisprudencia/jurisprudencia-stjd`
              : `/processos/editais`
          }
